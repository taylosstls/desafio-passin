...
export function Credential({ data, onChangeAvatar, onExpandQRCode }: Props) {
    ...
    return (
        <MotiView
            from={{ translateY: -height, rotateZ: '50deg', rotateY: '30deg', rotateX: '30deg' }}
            animate={{ translateY: 0, rotateZ: '0deg', rotateY: '0deg', rotateX: '0deg' }}
            ...
            <View className="bg-black/20 self-stretch items-center pb-6 border border-white/10 mx-3 rounded-2xl -mt-8">
                <ImageBackground source={require('@/assets/ticket/header.png')}
                    ...
                    <View className="w-full flex-row items-center justify-between">
                        <Text className="text-zinc-50 text-sm font-bold uppercase">{data.eventTitle}</Text>
                ...
                <Text className="text-zinc-300 text-sm font-bold mb-4">{data.email}</Text>
